...
method checkInputs {
	...
	$self->logError("package not defined") and exit if not defined $packagename;
...
method postInstall ($installdir, $version) {
	...
	$packagename = $self->repository() if not defined $packagename;
	$self->logDebug("username", $username);
	$self->logDebug("package", $packagename);
	$self->logDebug("installdir", $installdir);

	#### LOAD app AND parameter TABLE ENTRIES	
	$self->loadApps($login, $packagename, $installdir, $version);
...
method loadApps ($login, $packagename, $installdir, $version) {
#### LOAD app AND parameter TABLE ENTRIES
	...
	my $format	=	"yaml";
	$self->loadAppFiles($login, $packagename, $installdir, $appdir, $format);
